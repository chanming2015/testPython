...
#                     datas[index_expect] = json.dumps(jsob_expect.get('command'), ensure_ascii=False)
                    ...
                    if jsob_expect.get('command').get('dataFrom') is not None:
                        if jsob_reality.get('dataFrom') is not None:
                            ...
                            reality_command = {'api':jsob_reality.get('api'), 'dataFrom':jsob_reality.get('dataFrom'), 'param':jsob_reality.get('param')}
                            compare_command_all(expect_command, reality_command, datas, index_error)
                        else:
                            datas[index_error] = "预期有native，实际不是"
                            continue
                    else:
                        # 预期有command
                    ...
                    if jsob_reality.get('inspire') is not None:
                        if len(jsob_expect.get('inspire')) != len(jsob_reality.get('inspire')):
                            ...
                            for index, ins_expect in enumerate(jsob_expect.get('inspire')):
                                ...
                                if not compare_command_all(ins_expect, ins_reality, datas, index_error):
                                    break
                    else:
...
#                             if compare_command(command_map_expect, command_map_reality, datas, index_error):
#                                 datas[index_expect] = datas[index_reality_dm]
                else:
                    # 预期是nlg
